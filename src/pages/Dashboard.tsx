...
export default function Dashboard() {
  ...
  const clearCache = () => {
    localStorage.removeItem(getCacheKey('transactions'));
    localStorage.removeItem(getCacheKey('budgets'));
    localStorage.removeItem(getCacheKey('goals'));
    localStorage.removeItem(getCacheKey('investments'));
  };
  ...
  useEffect(() => {
    // Re-fetch data on currency change for fresh calculations
    fetchTransactions();
    fetchBudgets();
    fetchGoals();
  }, [currency]);
  ...
  const fetchTransactions = async () => {
    const cached = getCachedData<Transaction[]>('transactions');
    if (cached) {
  ...
  const fetchBudgets = async () => {
    const cached = getCachedData<any[]>('budgets');
    if (cached) {
      setBudgets(cached);
      return;
    }
    ...
    const data = await res.json();
    setBudgets(data);
    setCachedData('budgets', data);
  };

  const fetchGoals = async () => {
    const cached = getCachedData<any[]>('goals');
    if (cached) {
      setGoals(cached);
      return;
    }
    ...
    <div className="min-h-screen bg-bg-dark flex text-stone-100">
      ...
      <main className="flex-1 p-4 lg:p-8 max-w-7xl mx-auto w-full">
        <header className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-bold text-white">Habari, {auth.user?.email.split('@')[0]}!</h2>
            ...
            {activeTab === 'Dashboard' && (
              <>
                <section className="space-y-4">
                  ...
                  <PieChart2D data={chartData.length > 0 ? chartData : [{ category: 'No Data', amount: 1 }]} />
                </section>
