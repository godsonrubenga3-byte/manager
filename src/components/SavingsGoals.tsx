      ...
      <div className="glass p-8 rounded-3xl border border-white/5 light-theme:border-black/5 bg-white/[0.02] light-theme:bg-black/[0.01]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 mb-10">
          <input
            ...
            className="flex-1 px-4 py-3 bg-white/5 light-theme:bg-black/5 border border-white/10 light-theme:border-black/10 rounded-xl text-white light-theme:text-text-light text-sm outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary placeholder:text-stone-600 transition-all"
          />
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white/5 light-theme:bg-black/5 border border-white/10 light-theme:border-black/10 rounded-xl text-white light-theme:text-text-light text-sm outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary placeholder:text-stone-600 transition-all"
            />
          </div>
          <button
            type="submit"
        ...
        <div className="space-y-8">
          {goals.length === 0 ? (
            <div className="text-center py-10 text-stone-600 italic border-2 border-dashed border-white/5 light-theme:border-black/5 rounded-2xl">No savings goals yet. Start small!</div>
          ) : (
            goals.map((goal) => {
              ...
              return (
                <div key={goal.id} className="space-y-4 group">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-bold text-lg text-white light-theme:text-text-light flex items-center gap-2 mb-1">
                        {goal.name}
                        {isComplete && <CheckCircle2 className="w-5 h-5 text-emerald-500" />}
                      </h3>
                      <p className="text-xs font-mono text-stone-500 light-theme:text-stone-600 font-bold">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative group/input">
                        ...
                        <input
                            ...
                            className="w-32 pl-3 pr-3 py-2 bg-white/5 light-theme:bg-black/5 border border-white/10 light-theme:border-black/10 rounded-xl text-xs text-white light-theme:text-text-light outline-none focus:ring-2 focus:ring-primary/20 focus:pl-6 focus:border-primary transition-all placeholder:text-xs"
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                      ...
                      <button
                        ...
                        className="p-2 text-stone-700 light-theme:text-stone-600 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 lg:opacity-100"
                      >
                  ...
                  <div className="space-y-2">
                      <div className="h-3 bg-white/5 light-theme:bg-black/5 rounded-full overflow-hidden border border-white/5 light-theme:border-black/5 p-0.5">
                        <div
                      ...
                      <div className="flex justify-between items-center px-1">
                          <span className="text-xs text-stone-600 light-theme:text-stone-500 font-bold uppercase tracking-wider">{isComplete ? 'Goal Met!' : 'In Progress'}</span>
