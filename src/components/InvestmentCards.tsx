  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="px-3 py-1 bg-white/5 light-theme:bg-black/5 rounded-full border border-white/10 light-theme:border-black/10 text-xs font-bold text-stone-500 light-theme:text-stone-600 uppercase tracking-widest">
            Market Insights
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {displayInvestments.map((inv, idx) => (
          <div key={idx} className="glass p-6 rounded-3xl hover:border-primary/50 transition-all group relative overflow-hidden bg-white/[0.02] light-theme:bg-black/[0.01] border border-white/5 light-theme:border-black/5">
            <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <ArrowUpRight className="w-4 h-4 text-primary" />
            </div>
            
            <div className="flex justify-between items-start mb-4">
              <h3 className="font-bold text-lg text-white light-theme:text-text-light group-hover:text-primary transition-colors leading-tight max-w-[70%]">{inv.name}</h3>
            ...
            <p className="text-xs text-stone-400 light-theme:text-stone-600 mb-6 line-clamp-3 leading-relaxed font-medium">
                {inv.description}
            </p>
            
            <div className="grid grid-cols-2 gap-6 pt-6 border-t border-white/5 light-theme:border-black/5 bg-white/[0.01] light-theme:bg-black/[0.01] -mx-6 px-6 -mb-6 pb-6">
              <div>
                <div className="text-xs text-stone-600 light-theme:text-stone-500 uppercase font-bold tracking-widest mb-1.5">Est. Return</div>
                <div className="text-emerald-500 font-mono font-bold text-sm">{inv.expectedReturn}</div>
              </div>
              <div>
                <div className="text-xs text-stone-600 light-theme:text-stone-500 uppercase font-bold tracking-widest mb-1.5">Min. Amount</div>
                <div className="text-white light-theme:text-text-light font-mono font-bold text-sm">
